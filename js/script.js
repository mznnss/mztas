/* js/script.js — FINAL FIX (RECOVERY MODE) */
...
/* ================= 2. INIT & DATA HANDLING (RECOVERY LOGIC) ================= */
// SAYA GANTI KEY-NYA AGAR BROWSER DIPAKSA MEMBUAT DATA BARU
const STORAGE_DB_KEY = "mz_storage_RECOVERY_MODE"; 
...
try {
    ...
    // 2. PENGECEKAN KRUSIAL:
    // Jika db masih kosong (0) atau null, PAKSA ambil dari file db.js
    if (!db || db.length === 0) {
        if (typeof produk !== "undefined") {
            console.log("Mengambil data dari db.js...");
            db = [...produk]; // Salin data dari file db.js
            localStorage.setItem(STORAGE_DB_KEY, JSON.stringify(db)); // Simpan ke HP
        } else {
            console.error("FATAL: File db.js tidak terbaca!");
        }
    }
} catch (err) {
    console.error("Resetting database due to error...");
    // Jika error, fallback ke db.js
...
function createCardHTML(p) {
    // Cek jika data produk rusak
    if (!p || !p.variasi) return '';
...
function renderProducts(filter = 'all') {
    ...
    // Cek lagi, jika db masih kosong, paksa reload
    if (!db || db.length === 0) {
        if(typeof produk !== 'undefined') {
             db = produk; // Fallback terakhir
             saveDB();
        } else {
             container.innerHTML = `<div class="col-span-full text-center py-20 text-red-400">Error: Data produk tidak ditemukan. Pastikan db.js dimuat.</div>`;
             return;
        }
    }
...
function completeOrder(e) {
    ...
    cart.forEach(cartItem => {
        ...
        if (productInDB) {
            ...
            if (varcurrentProduct = db.find(p => p.id === id);
    if (!currentProduct) return;
