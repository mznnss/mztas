...
el.innerHTML = fav
.map(
(p) =>   <div class="bg-white rounded-xl shadow-lg overflow-hidden transform hover:scale-105 transition cursor-pointer"   onclick="openProductDetail(${p.id})">   <div class="h-52 overflow-hidden bg-gray-100">   <img src="img/produk/${p.gambar}" class="w-full h-full object-cover">   </div>   <div class="p-4">   <h3 class="font-bold">${p.nama}</h3>   <div class="text-blue-700 font-semibold mt-2">${formatIDR(   getLowestPrice(p)   )}</div>   </div>   </div>  
)
.join("");
}
...
: "";
return   <div class="relative bg-white rounded-xl shadow p-3 cursor-pointer hover:shadow-lg transition" onclick="openProductDetail(${   p.id   })">   ${badge}   <div class="h-44 overflow-hidden rounded-lg">   <img src="img/produk/${p.gambar}" class="w-full h-full object-cover">   </div>   <h3 class="font-semibold mt-3">${p.nama}</h3>   <div class="text-blue-700 font-bold mt-1">${formatIDR(lowest)}</div>   <div class="mt-3 text-sm text-gray-600">Klik untuk lihat detail</div>   </div>  ;
})
.join("");
}
...
body.innerHTML = cart
.map(
(it, idx) =>   <div class="flex items-center justify-between gap-4 border-b py-3">   <div class="flex items-center gap-3">   <img src="img/produk/${   it.gambar   }" class="w-16 h-16 object-cover rounded">   <div>   <div class="font-semibold">${it.nama}</div>   <div class="text-sm text-gray-600">${it.ukuran} • ${it.warna}</div>   <div class="text-blue-700 font-bold mt-1">${formatIDR(   it.harga   )}</div>   </div>   </div>   <div class="text-right">   <div class="flex items-center gap-2 justify-end">   <button onclick="changeCartQty(${idx}, -1)" class="px-2 py-1 border rounded">-</button>   <div>${it.qty}</div>   <button onclick="changeCartQty(${idx}, 1)" class="px-2 py-1 border rounded">+</button>   </div>   <button onclick="removeCartItem(${idx})" class="text-sm text-red-500 mt-2">Hapus</button>   </div>   </div>  
)
.join("");
...
cart
.map(
(it) =>   <div class="border-b py-3">   <div class="flex justify-between">   <div>   <div class="font-semibold">${it.nama}</div>   <div class="text-sm text-gray-600">${it.ukuran} • ${it.warna}</div>   <div class="text-gray-600">Jumlah: ${it.qty}</div>   </div>   <div class="font-semibold">${formatIDR(it.qty * it.harga)}</div>   </div>   </div>  
)
.join("") +
<div class="mt-4 text-right font-bold">${formatIDR(   cart.reduce((s, i) => s + i.harga * i.qty, 0)   )}</div>;
}
...
el.innerHTML = rev
.map(
(r) =>   <div class="bg-white p-5 rounded-xl shadow">   <div class="flex items-center gap-3 mb-2">   <div class="w-12 h-12 bg-blue-200 rounded-full flex items-center justify-center">${   r.icon || "👤"   }</div>   <div>   <div class="font-semibold">${r.name}</div>   <div class="text-xs text-gray-500">${r.date}</div>   </div>   </div>   <div class="font-semibold">${r.title}</div>   <div class="text-gray-700 mt-1">${r.text}</div>   <div class="text-yellow-500 mt-2">${"★".repeat(r.rating)}</div>   </div>  
)
.join("");
}
...
el.innerHTML = db
.map(
(p) =>   <div class="bg-white p-4 rounded shadow mb-3">   <div class="flex items-start gap-4">   <img src="img/produk/${   p.gambar   }" class="w-20 h-20 object-cover rounded">   <div class="flex-1">   <div class="font-bold">${p.nama}</div>   <div class="text-sm text-gray-600">ID: ${p.id}</div>   <div class="mt-2">   ${p.variasi   .map(   (v, idx) =>
